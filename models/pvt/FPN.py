...
class FPN(nn.Module):
    ...
    def __init__(self, num_classes, back_bone='resnet', pretrained=True):
        ...
        self.back_bone = pvt_tiny()
        ckpt = torch.load('/hpc/users/CONNECT/xuzheng/C2VKD/models/pvt/pvt_tiny.pth',map_location='cpu')
        ...
        # KD branch
        self.conv_kd = nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # [2, 512, 16, 16] -> [2, 256, 32, 32]
        feat = self.conv_kd(low_level_features[3])
        feat = F.interpolate(feat, size=(32,32), mode='bilinear', align_corners=True)
